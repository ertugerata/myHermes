...
Bu kılavuz, **Hermes Agent** web arayüzünün (Dashboard) Hugging Face Spaces veya yerel bir Docker ortamında nasıl kurulacağını, çalıştırılacağını, gelişmiş ağ (DNS) çözümlerini, güvenlik yapılandırmalarını, hata takip yöntemlerini ve başlangıç performansı iyileştirmelerini detaylandırmaktadır.
...
## ⏳ Başlangıç Aşaması Performans Analizi (Neden Uzun Sürüyor?)

Hugging Face Spaces üzerinde derleme sonrası (factory build veya yeniden başlatma) uygulamanın ilk kez ayağa kalkması ve "Running" durumuna geçmesi bazen **1-3 dakika** kadar sürebilir. Bu durum bir hata değildir ve aşağıdaki kritik adımların sırayla yürütülmesinden kaynaklanır:

1. **GitHub Yedek Deposunun Klonlanması (En Büyük Zaman Kaynağı):**
   - Başlangıçta, geçmiş oturumlarınızın ve ayarlarınızın kaybolmaması için `GITHUB_BACKUP_REPO` adresindeki yedek deposu `git clone --depth 1` ile yerel konteynere indirilir.
   - Eğer yedek deponuzda çok fazla geçmiş oturum birikmişse veya `.gitignore` kuralları uygulanmadan önce depoya büyük boyutlu ikili (binary) dosyalar commit edilmişse, bu klonlama ve dosya kopyalama işlemi ağ hızına bağlı olarak zaman alabilir.
2. **DNS-over-HTTPS (DoH) Ön Çözümleme Aşaması:**
   - Hugging Face'in DNS engellemelerini aşmak için `scripts/dns-resolve.py` arka planda çalıştırılarak engelli tüm popüler servislerin (Telegram, WhatsApp, OpenAI vb.) IP adresleri Cloudflare/Google DNS üzerinden çözümlenir.
   - Bu süreçte her alan adı için sıralı ağ istekleri yapıldığından, Hugging Face sunucularının o anki ağ yoğunluğu başlangıç süresini doğrudan etkiler.
3. **Dinamik Kimlik Doğrulama ve Şifre Hash'leme (CPU Yoğun):**
   - Konteyner ayağa kalkarken şifreniz (`HERMES_DASHBOARD_BASIC_AUTH_PASSWORD`), Hermes'in yerleşik güvenli `scrypt` algoritması kullanılarak tek seferlik hash'lenir.
   - Hugging Face'in ücretsiz/düşük CPU'lu konteynerlerinde bu kriptografik işlem birkaç saniye ek yük getirebilir.
4. **Çalışma Alanı Hazırlığı:**
   - Konfigürasyon dosyalarının doğru arama yollarına (`~/.config/hermes` ve `~/.hermes`) kopyalanması ve izinlerin set edilmesi adımları gerçekleştirilir.

---

## 🔍 Hata ve Sıkıntı Takibi (Sorunları Nasıl Görebilirim?)

Uygulamanın ayağa kalkamaması, çömesi veya beklenmeyen bir davranış sergilemesi durumunda sorunları tespit etmek için aşağıdaki adımları izleyebilirsiniz:

### 1. Hugging Face Canlı Log Paneli (En Kolay Yöntem)
- Hugging Face Space sayfanızın sağ üst köşesinde yer alan **"Logs"** butonuna tıklayın.
- Burada konteynerin Docker build aşamasından, `start.sh` başlangıç betiğinin tüm çıktılarına kadar her şeyi canlı olarak izleyebilirsiniz.
- Başarılı bir açılışta aşağıdaki gibi loglar görmelisiniz:
  ```text
  === DNS HAZIRLIĞI VE ÖNÇÖZÜMLEME ===
  ✔ PYTHONPATH ayarlandı: ...
  === VERİ GERİ YÜKLEME AŞAMASI (GITHUB BACKUP) ===
  ✔ Yedek deposu başarıyla klonlandı.
  ✔ .hermes geri yüklendi.
  ✔ config.yaml doğru konumlarda hazır.
  === HERMES AGENT BAŞLATILIYOR ===
  Dinlenen Port: 7860
  [dns-fix] Python socket.getaddrinfo patched successfully
  ```

### 2. Kritik Dosyaları ve Geçici Logları Kontrol Etme
Hata durumunda, `start.sh` tarafından üretilen geçici log dosyaları incelenebilir:
- **/tmp/git_clone.log:** GitHub yedek deposunun klonlanması sırasındaki hataları (örn. hatalı token veya geçersiz depo adresi) içerir.
- **/tmp/git_push.log:** Periyodik yedekleme veya güvenli kapanış esnasında yedeklerin GitHub'a yüklenirken karşılaştığı hataları içerir.
- **auth_config_output.log:** İlk başlatmadaki kimlik doğrulama ayarlarını yapan Python kodunun hata çıktılarını içerir (hata oluşursa otomatik olarak ekrana basılır).
- **/tmp/dns-resolved.json:** DoH çözümleyicinin başarıyla çözdüğü alan adlarını ve IP eşleşmelerini listeler.

### 3. Sık Karşılaşılan Sorunlar ve Çözümleri
- **❌ Depo Klonlanamadı Hatası:** `GITHUB_TOKEN` veya `GITHUB_BACKUP_REPO` değerlerinizin doğruluğunu kontrol edin. Token'ın en azından `repo` yazma/okuma yetkisine sahip olduğundan emin olun.
- **❌ Dashboard'a Erişilemiyor (Bağlantı Hatası):** Port ayarlarının doğru yapıldığından emin olun. Hugging Face Spaces varsayılan olarak `7860` portunu dinler; bu portu manuel olarak değiştirmeyin.
- **❌ "basic auth provider failed to register" Hatası:** `config.yaml` dosyanızda basic authentication eklentisinin (`basic`) eklenti listesinde doğru yapılandırıldığından emin olun. `start.sh` bu ayarı otomatik olarak tamir etmeye çalışır.
...
| `GITHUB_TOKEN` | Sır (Secret) | *(Boş)* | GitHub deponuza yazma yetkisi olan kişisel erişim token'ı (Personal Access Token - PAT). Yedeklerin depoya push edilebilmesi için gereklidir. |
